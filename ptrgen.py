    ...
    try:
        for aid, article in enumerate(tqdm(dataset['data'])):
            for pid, paragraph in enumerate(article['paragraphs']):
                ...
                story = TokenString(__(paragraph['context']), word_tokenize).delete_whitespace()
                    ...
                    if not indices:
                        ...
                        log.error('{}.{}.{} - "{}" not found in \n"{}"'
                                  .format(aid, pid, qid, a.tokenized_string,
                                          story.tokenized_string))
                        skipped += 1
                        continue
                        
                    a_start, a_end = indices
                    fields = (aid, pid, qid, squad_id, story, q, a)
    ...
    try:
        ...
        for e in range(eons):
            ...
            for ri in tqdm(range(predictor_feed.num_batch//100)):
                output, _results = predictor.predict(ri, 3)
...
if __name__ == '__main__':
    ...
    if Config.flush:
        ...
        dataset, vocabulary = load_squad_data('dataset/train-v1.1.json', ids, max_sample_size=10)
    ...
    log.info('dataset[:10]: {}'.format(pformat(dataset)))
